...
export async function generateStaticParams() {
  ...
  return posts.map((p) => ({ category: p.category?.slug || 'uncategorized', slug: p.slug }));
}
...
export async function generateMetadata({ params }: { params: { category: string; slug: string } }) {
  ...
  return {
    ...
    openGraph: { title: post.title, description: post.subtitle, images: post.heroImage ? [{ url: post.heroImage }] : [] },
  };
}
...
export default async function ArticlePage({ params }: { params: { category: string; slug: string } }) {
  ...
  const relatedPosts = related.filter((p) => p.slug !== post.slug && p.category?.slug === post.category?.slug).slice(0, 3);
  ...
  return (
    <article className="container-page py-6 sm:py-8">
      ...
      <nav className="mb-4 text-sm text-muted-foreground">
        ...
        <Link href={`/category/${post.category?.slug}`} className="hover:text-foreground">{post.category?.name}</Link>
      </nav>

      {post.category && <span className="text-sm font-semibold uppercase tracking-wider text-primary">{post.category.name}</span>}
      ...
      <div className="mt-4 flex items-center gap-4 text-sm text-muted-foreground">
        {post.author && (
          <div className="flex items-center gap-2">
            {post.author.avatar && <img src={post.author.avatar} alt={post.author.name} className="h-8 w-8 rounded-full object-cover" />}
      ...
      {post.heroImage && (
        <div className="mt-6 overflow-hidden rounded-2xl">
          <img src={post.heroImage} alt={post.title} className="aspect-[16/9] w-full object-cover" />
        </div>
      )}
      ...
      {post.content && (
        <div className="prose-article mt-8 max-w-3xl text-lg leading-relaxed">
          {post.content.split('\n').map((line, i) => {
            if (line.startsWith('## ')) return <h2 key={i} className="mt-8 font-serif text-2xl font-bold">{line.slice(3)}</h2>;
      ...
      {post.tags && post.tags.length > 0 && (
        <div className="mt-8 flex flex-wrap gap-2">
          {post.tags.map((tag) => <span key={tag.id} className="rounded-full bg-muted px-3 py-1 text-xs font-medium text-muted-foreground">#{tag.name}</span>)}
        </div>
      )}
      ...
      {relatedPosts.length > 0 && (
        <section className="mt-12 border-t border-border pt-8">
          ...
          <div className="mt-4 grid grid-cols-1 gap-4 sm:grid-cols-3">
            {relatedPosts.map((p) => <CompactCard key={p.id} post={p} />)}
          </div>
        </section>
      )}
